...
public class UserService {
	...
	public ResponseEntity<?> findUsers(UserRequestDto request){
		List<?> users;
		if(Boolean.TRUE.equals(request.getFlatten())){
	...
	public List<UserGetFlattenDto> findUsersflatten(){
		List<User> users=userRepository.findAll();
		List<UserGetFlattenDto> usersDto = new ArrayList<>();
		System.out.println("4");
		if(users.stream().count()>0){
			for(User user : users){
				UserGetFlattenDto userGetFlattenDto=modelMapper.map(user,UserGetFlattenDto.class);
				usersDto.add(userGetFlattenDto);
			}
		}
		return usersDto;
	}
